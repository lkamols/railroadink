...
#        
        ...
        for row, col, in [(4,3),(5,3),(6,3),(6,4),(6,5)]:
            solution_overlay = Image.new("RGB", (SQUARE_WIDTH//4, SQUARE_HEIGHT//4), color="green")
            im.paste(solution_overlay, (LEFT_OFFSET + BOARD_WIDTH * col // NUM_COLS + 3*SQUARE_WIDTH//4, 
                                        TOP_OFFSET + BOARD_HEIGHT * row // NUM_ROWS))
            
            
        colours = ['purple', 'yellow', 'blue', 'red']
        count = 0
        for cluster_rep in self._cluster_reps:
            if not cluster_rep.is_free_blank_cluster() and cluster_rep.get_start_count() > 0 and len(cluster_rep.get_cluster_tiles()) > 1:
                #take off the tile element, we only care about locations
                for row, col, tile in cluster_rep.get_cluster_tiles():
                    if row < 0 or row >= NUM_ROWS or col < 0 or col >= NUM_COLS:
                        continue
                    solution_overlay = Image.new("RGB", (SQUARE_WIDTH//4, SQUARE_HEIGHT//4), color=colours[count])
                    if count == 0: 
                        im.paste(solution_overlay, (LEFT_OFFSET + BOARD_WIDTH * col // NUM_COLS, 
                                            TOP_OFFSET + BOARD_HEIGHT * row // NUM_ROWS))
                    else:
    ...
    def _check_for_illegal_clusters(self, cluster_reps):
        return
        #COMMENTED OUT AS IT DOESN'T DEAL WITH CLUSTERS STARTED AT OVERPASSES, WOULD NEED TO FIX IF THIS CODE GETS USED
